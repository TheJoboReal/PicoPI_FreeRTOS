...
void move_stepper(StepperMotor* motor, int steps, int direction_step, int delay_us) {
    for (int i = 0; i < steps; i++) {
        ...
        sleep_us(delay_us);  // Step delay in microseconds
...
int main() {
    ...
    // Define pins for both motors (left 0-3, right 4-7)
    uint pins[] = {0, 1, 2, 3, 4, 5, 6, 7};
    StepperMotor motor;

    // Initialize stepper motor with 8 pins, half step mode, 15% PWM, micro-steps
    init_stepper(&motor, pins, 1, 15, MICRO_STEPS);

    // Move both motors forward
    move_stepper(&motor, 200, 1, STEP_DELAY_US);  // Move both forward with 1 ms delay

    // Move both motors backward
    move_stepper(&motor, 200, -1, STEP_DELAY_US);  // Move both backward with 1 ms delay
